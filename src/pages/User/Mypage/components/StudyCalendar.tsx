...
const TYPE_BADGE: Record<string, string> = {
  event: "bg-red-400 text-white",
  academic: "bg-blue-500 text-white",
  personal: "bg-amber-400 text-gray-800",
};
  ...
  return (
    <div className="border border-gray-200 rounded-xl p-6 mb-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <a href="/mycalendar" className="text-xs text-blue-500 hover:underline">
          전체일정 ›
        </a>
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-center gap-4 mb-4">
            <button
              onClick={() =>
                setCurrent((p) => (p.month === 1 ? { year: p.year - 1, month: 12 } : { ...p, month: p.month - 1 }))
              }
            ...
            <button
              onClick={() =>
                setCurrent((p) => (p.month === 12 ? { year: p.year + 1, month: 1 } : { ...p, month: p.month + 1 }))
              }
              className="text-gray-400 hover:text-gray-700 px-1"
            >
              ›
              ...
              return (
                ...
                >
                  <span
                    ...
                    ${isToday ? "bg-gray-900 text-white" : col === 0 ? "text-red-400" : col === 6 ? "text-blue-400" : "text-gray-700"}`}
                  >
                    {day}
                  </span>
                  {hasDot(day) && <span className="w-1 h-1 rounded-full bg-red-400 mt-0.5" />}
                </button>
              );
            })}
          </div>
        ...
        <div className="w-72 flex-shrink-0">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/mycalendar" className="text-xs text-gray-400 hover:text-blue-500">
              일정 추가 +
            </a>
          </div>
