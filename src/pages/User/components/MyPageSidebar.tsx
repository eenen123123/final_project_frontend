import { useState } from 'react';

interface NavItem {
  label: string;
  href: string;
  children?: { label: string; href: string }[];
}

const NAV: NavItem[] = [
  { label: '수강중 강좌', href: '/mylecture' },
  { label: 'My 캘린더',   href: '#calendar' },
  { label: '주문/배송 조회', href: '#order' },
  { label: '수강 신청',   href: '#enroll' },
  {
    label: '내 강의실',
    href: '#classroom',
    children: [
      { label: '수강중 강좌', href: '/mylecture' },
      { label: '강좌 수강신청', href: '#enroll' },
      { label: '다이렉트 교재신청', href: '#book' },
      { label: '수강 이력 관리', href: '#history' },
    ],
  },
  {
    label: '주문·결제·혜택',
    href: '#payment',
    children: [
      { label: '장바구니', href: '#cart' },
      { label: '주문/배송 조회', href: '#order' },
      { label: '이용권 관리', href: '#voucher' },
      { label: '쿠폰/포인트', href: '#coupon' },
    ],
  },
  {
    label: '나의 정보',
    href: '#info',
    children: [
      { label: '학습 알림 설정', href: '#notification' },
      { label: '개인 정보 수정', href: '#profile' },
    ],
  },
];

interface MyPageSidebarProps {
  activeSection: string;
  onSectionChange: (section: string) => void;
}

export default function MyPageSidebar({ activeSection, onSectionChange }: MyPageSidebarProps) {
  const [openMenus, setOpenMenus] = useState<string[]>(['내 강의실', '주문·결제·혜택', '나의 정보']);

  const toggleMenu = (label: string) => {
    setOpenMenus((prev) =>
      prev.includes(label) ? prev.filter((l) => l !== label) : [...prev, label]
    );
  };

  return (
    <aside className="w-48 flex-shrink-0">
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-blue-600">마이페이지</h2>
        </div>
        <nav className="py-2">
          {NAV.map((item) => (
            <div key={item.label}>
              <button
                onClick={() => {
                  if (item.children) toggleMenu(item.label);
                  else onSectionChange(item.label);
                }}
                className={`w-full flex items-center justify-between px-5 py-2.5 text-sm transition-colors rounded-md ${
                  openMenus.includes(item.label) || activeSection === item.label
                    ? 'text-blue-600 font-semibold bg-blue-50'
                    : 'text-gray-700 hover:bg-gray-50'
                }`}
              >
                <span>{item.label}</span>
                {item.children ? (
                  <span className="text-gray-400 text-xs">
                    {openMenus.includes(item.label) ? '∧' : '∨'}
                  </span>
                ) : (
                  <span className="text-gray-300 text-xs">›</span>
                )}
              </button>

              {/* 서브메뉴 — max-height*/}
              {item.children && (
                <div
                  className="overflow-hidden transition-all duration-300 ease-in-out"
                  style={{
                    maxHeight: openMenus.includes(item.label)
                      ? `${item.children.length * 36}px`
                      : '0px',
                  }}
                >
                  <div className="bg-gray-50 py-1">
                    {item.children.map((child) => (
                      <a
                        key={child.label}
                        href={child.href}
                        className="block px-8 py-2 text-xs text-gray-500 hover:text-blue-600 transition-colors"
                      >
                        {child.label}
                      </a>
                    ))}
                  </div>
                </div>
              )}
            </div>
          ))}
        </nav>
      </div>
    </aside>
  );
}
