          ...
          <div className="flex-1 min-w-0 w-full">
                    ...
                    {loading ? (
                      ...
                      <tr key={item.postSn} className="hover:bg-gray-50 transition-colors">
                        <td className="py-3 px-3 text-center">
                          {item.topFixYn === 'Y' ? (
                            <span className="inline-block bg-blue-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded">BEST</span>
                          ) : (
                        ...
                        <td className="py-3 px-3">
                          <Link to={`/customer/faq/${item.postSn}`}
                      ...
                      <tr><td colSpan={4} className="py-12 text-center text-sm text-gray-400">검색 결과가 없습니다.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
                  ...
                  <Link key={item.postSn} to={`/customer/faq/${item.postSn}`}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="text-[10px] text-gray-400">{item.faqCtgNm}</span>
                        <span className="text-[10px] text-gray-300">·</span>
                        <span className="text-[10px] text-gray-400">{item.faqSubCtgNm}</span>
                        {item.topFixYn === 'Y' && (
                          <span className="text-[10px] bg-blue-600 text-white font-bold px-1 py-0.5 rounded">BEST</span>
                        )}
                      </div>
                      <p className="text-sm text-gray-800">{item.postSj}</p>
                    </div>
            ...
            <div className="border border-gray-200 rounded-sm p-5 bg-white text-center">
              <p className="text-xs text-gray-600 mb-5">
                원하는 답변을 찾지 못하셨나요?{' '}
                <Link to="/customer/qna" className="text-blue-600 font-bold underline decoration-blue-600 underline-offset-2">
                  1:1 상담으로 도와드릴게요.
                </Link>
              </p>
              <div className="flex flex-col sm:flex-row justify-center items-center gap-6 sm:gap-16">
                ...
                <Link to="/customer/qna" className="flex items-center gap-3 text-left group cursor-pointer">
