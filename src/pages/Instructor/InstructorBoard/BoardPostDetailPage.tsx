...
export default function BoardPostDetailPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((e) => { if (!cancelled) console.error("게시글 상세 조회 실패", e); })
  ...
  return (
    <div>
        ...
        post.answerYn === "Y" && post.answerContent ? (
          <div className="mb-6 border border-blue-100 rounded">
            <div className="px-4 py-3 bg-blue-50 border-b border-blue-100 flex items-center gap-3">
              ...
              {post.answererName && (
                <span className="text-xs text-gray-500">{post.answererName}</span>
              )}
              {post.answerDt && (
                <span className="text-xs text-gray-400">{formatPostDate(post.answerDt)}</span>
              )}
            </div>
      ...
      <div className="border-t border-gray-200">
        <div
          onClick={() => post.prevPost && navigate(`/instructor/${instrUuid}/${boardType}/${post.prevPost.postSn}`)}
        ...
        <div
          onClick={() => post.nextPost && navigate(`/instructor/${instrUuid}/${boardType}/${post.nextPost.postSn}`)}
