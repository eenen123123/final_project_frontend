...
interface Question {
  qstnSn: number; qstnNo: number; qstnCn: string;
  qstnType: QuestionType; score: number; choices: string[] | null;
}

interface ExamDetail {
  examSn: number; examNm: string; examEndDt: string; questions: Question[];
}
...
function useCountdown(endDt: string | null) {
  ...
  useEffect(() => {
    if (!endDt) return;
    const tick = () => { const diff = Math.max(0, new Date(endDt).getTime() - Date.now()); setRemaining(diff); };
    tick();
