        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="px-7 py-6 border-b border-slate-200">
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-3 text-sm text-slate-400">
              ...
              {item.inqCnt != null && <span className="whitespace-nowrap">조회 {item.inqCnt}</span>}
              <span className="whitespace-nowrap">{item.regDt ? item.regDt.slice(0, 16).replace("T", " ") : "-"}</span>
            </div>
          </div>
          ...
          {content && (
            <RichContent
              html={content}
              className="px-7 py-8 text-sm text-slate-700 leading-relaxed min-h-[140px] prose prose-sm max-w-none border-b border-slate-200"
            />
          )}
