  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-3.5 flex items-center gap-3 cursor-pointer hover:bg-blue-100/70 transition-colors">
        <div className="w-9 h-9 bg-blue-600 rounded-lg flex items-center justify-center text-white text-base flex-shrink-0">💡</div>
        <div className="flex-1 min-w-0">
          <p className="text-[11px] text-blue-500 font-bold">오늘의 문제 · 영어</p>
          <p className="text-sm font-bold text-blue-900 mt-0.5 truncate">관계대명사 who와 which의 차이를 설명하시오</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-2 bg-white border border-slate-200/80 rounded-2xl flex flex-col justify-between shadow-xs overflow-hidden">
          <div className="bg-sky-50/60 px-5 py-3.5 border-b border-slate-200/40">
            <h3 className="text-xs font-black text-slate-800 tracking-tight">이번 주 학습 시간</h3>
            <p className="text-[11px] text-slate-400 mt-0.5">지난주 대비 <span className="text-blue-600 font-bold">30분 더</span> 공부했어요</p>
          </div>

          <div className="p-5 pt-7 flex items-end justify-between px-7 h-32">
            {weeklyStudyData.map((data, index) => {
              const heightPercent = data.time > 0 ? (data.time / maxTime) * 100 : 4;
              return (
                    ...
                    <motion.div
                      ...
                      className={`w-full rounded-t-md transition-colors ${
                        data.time === maxTime 
                          ...
                          : data.time === 0 ? "bg-slate-200" : "bg-blue-500 group-hover:bg-blue-600"
                      }`}
                    />
                  </div>
                  <span className="text-[11px] font-bold text-slate-400 group-hover:text-slate-800 transition-colors">{data.day}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl flex flex-col justify-between shadow-xs overflow-hidden">
          <div className="bg-sky-50/60 px-5 py-3.5 border-b border-slate-200/40">
            <h3 className="text-xs font-black text-slate-800 tracking-tight">학습 달성률</h3>
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 shadow-xs">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs font-black text-slate-800 tracking-tight">최근 공지사항</h3>
          <button onClick={() => onTabChange("notice")} className="text-[11px] font-bold text-blue-600 hover:underline">
            전체보기
          </button>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 cursor-pointer hover:bg-slate-100/50 transition-colors">
          <div className="flex items-center gap-2">
            <span className="text-[9px] bg-red-50 text-red-500 px-1.5 py-0.5 rounded font-bold">필독</span>
            <h4 className="text-xs font-bold text-slate-700 truncate flex-1">5월 중간고사 일정 및 고사장 안내</h4>
          </div>
          <p className="text-[10px] text-slate-400 mt-1.5 font-medium">2026.05.15 · 김민준 선생님</p>
        </div>
      </div>
